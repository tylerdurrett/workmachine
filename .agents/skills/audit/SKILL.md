...
For each unified finding, check whether the issue is actually addressed by a *later* open child — or already shipped by a closed child. Dispatch a `Task(subagent_type="Explore")` subagent per finding (run them in parallel — one Task call per finding, all dispatched in the same response). Brief the subagent with the finding's claim, its cited targets, and every child's body (from Step 2). Ask it to report whether any later open child's `## Scope`, `## Sub-tasks`, `## Candidate tasks`, `## Acceptance criteria`, `## User Stories`, or `## Definition of done` covers the same work, or whether a closed child already addressed it.
